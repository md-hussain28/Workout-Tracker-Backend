...
async def exercise_stats(
    ...
    total_sets = counts.total_sets or 0
    total_workouts = counts.total_workouts or 0
    ...
    first_performed = dates.first.isoformat() if dates.first else None
    last_performed = dates.last.isoformat() if dates.last else None
    ...
    best_reps = int(prs.best_reps) if prs.best_reps else None
    best_duration = int(prs.best_duration) if prs.best_duration else None
    ...
    # Group by workout date, keep best 1RM per workout
    ...
    recent_ids = [r[0] for r in recent_workout_ids_result.all()]
    recent_history = []
    if recent_ids:
                ...
                "sets": [
                    {
                        ...
                        "duration_seconds": int(s.duration_seconds) if s.duration_seconds else None,
                        "set_label": s.set_label.value if s.set_label else None,
                        "is_pr": s.is_pr,
                    }
                    for s in sorted(ex_sets, key=lambda x: x.set_order)
                ],
            })
